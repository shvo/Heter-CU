...
pipe float p13 __attribute__((xcl_reqd_pipe_depth(16)));
pipe float p14 __attribute__((xcl_reqd_pipe_depth(16)));
pipe float p15 __attribute__((xcl_reqd_pipe_depth(16)));
pipe float p16 __attribute__((xcl_reqd_pipe_depth(16)));
...
void runJacobi1D_kernel6(__global DATA_TYPE* A, __global DATA_TYPE* B)
{
        ...
        write_pipe_block(p11, &A_local[N-2]);
        write_pipe_block(p11, &A_local[N-1]);
        ...
        for (t = 1; t <= T; ++t) {
            ...
            read_pipe_block(p12, &B_local[N-1]);
...
__kernel __attribute__ ((reqd_work_group_size(1,1,1)))
void runJacobi1D_kernel7(__global DATA_TYPE* A, __global DATA_TYPE* B)
{
        __local DATA_TYPE A_local[N] __attribute__((xcl_array_partition(cyclic,4,1)));
        __local DATA_TYPE B_local[N] __attribute__((xcl_array_partition(cyclic,4,1)));
        ...
        write_pipe_block(p13, &A_local[0]);
        write_pipe_block(p13, &A_local[1]);
        write_pipe_block(p13, &A_local[N-2]);
        write_pipe_block(p13, &A_local[N-1]);
        ...
        for (t = 1; t <= T; ++t) {
            ...
            read_pipe_block(p14, &B_local[0]);
            read_pipe_block(p14, &B_local[N-1]);
...
__kernel __attribute__ ((reqd_work_group_size(1,1,1)))
void runJacobi1D_kernel8(__global DATA_TYPE* A, __global DATA_TYPE* B)
{
        __local DATA_TYPE A_local[N] __attribute__((xcl_array_partition(cyclic,4,1)));
        __local DATA_TYPE B_local[N] __attribute__((xcl_array_partition(cyclic,4,1)));
        ...
        write_pipe_block(p15, &A_local[0]);
        write_pipe_block(p15, &A_local[1]);
...
void runJacobi1D_connect_1_4(int foo)
{
    ...
    float tmp15, tmp16, tmp17, tmp18, tmp19, tmp20, tmp21;
    float tmp22, tmp23, tmp24, tmp25, tmp26, tmp27, tmp28;
    //while(1) {
        // read from kernel1
        ...
        read_pipe_block(p11, &tmp21);
        read_pipe_block(p11, &tmp22);

        // read from kernel7
        read_pipe_block(p13, &tmp23);
        read_pipe_block(p13, &tmp24);
        read_pipe_block(p13, &tmp25);
        read_pipe_block(p13, &tmp26);

        // read from kernel8
        read_pipe_block(p15, &tmp27);
        read_pipe_block(p15, &tmp28);
        ...
        tmp22 = 0.33333f * (tmp21 + tmp22 + tmp23);
        tmp23 = 0.33333f * (tmp22 + tmp23 + tmp24);
        tmp26 = 0.33333f * (tmp25 + tmp26 + tmp27);
        tmp27 = 0.33333f * (tmp26 + tmp27 + tmp28);
        ...
        write_pipe_block(p12, &tmp22);

        // write to kernel7
        write_pipe_block(p14, &tmp23);
        write_pipe_block(p14, &tmp26);

        // write to kernel8
        write_pipe_block(p16, &tmp27);
    //}
}
