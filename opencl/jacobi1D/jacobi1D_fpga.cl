...
void runJacobi1D_kernel1(__global DATA_TYPE* A, __global DATA_TYPE* B)
{
        ...
        __local DATA_TYPE B_local[N];
        int g_id = get_group_id(0);
        if ( g_id == 0 ) {
          async_work_group_copy(A_local, A, N+1, 0);
        } 
        else if ( g_id > 0 & g_id < M/N - 1 ) {
          async_work_group_copy(A_local, &A[N*g_id-1], N+2, 0);
        }
        else if ( g_id == M/N - 1 ) {
          async_work_group_copy(A_local, &A[N*g_id-1], N+1, 0);
        }
