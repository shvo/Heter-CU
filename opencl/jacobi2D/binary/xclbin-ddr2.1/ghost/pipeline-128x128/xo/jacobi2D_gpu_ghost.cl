...
#define X 128   // local 0-dimension size
#define Y 128   // local 1-dimension size
...
{
    //__local DATA_TYPE A_local[(X+2*T)*(Y+2*T)] __attribute__((xcl_array_partition(cyclic,16,1)));
    //__local DATA_TYPE B_local[(X+2*T)*(Y+2*T)] __attribute__((xcl_array_partition(cyclic,16,1)));
