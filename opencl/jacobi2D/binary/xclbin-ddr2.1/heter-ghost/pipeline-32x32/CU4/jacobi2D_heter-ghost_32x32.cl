...
void runJacobi2D_kernel1(__global DATA_TYPE* A, __global DATA_TYPE* B, int n)
{
    ...
    if (gid_y == 0) {
        if (gid_x == 0) {
            for (i = 0; i < Y; ++i) {
                async_work_group_copy(&A_local[X*i], &A[(gid_y*Y+i)*M + gid_x*X], X, 0);
            }
        }
        ...
        if (gid_x == 0) {
            for (i = 0; i < Y + T; ++i) {
                async_work_group_copy(&A_local[X*i], &A[(gid_y*Y+i-T)*M + gid_x*X], X, 0);
            }
        }
        else if (gid_x < M/(2*X)-1) {
            for (i = 0; i < Y + T; ++i) {
    ...
    for (t = 1; t <= T; ++t) { 
        if (gid_y == 0) {
            if (gid_x == 0) { 
                ...
                // write to pipe
                for (j = 0; j <= Y-1; ++j) {
                    write_pipe_block(p2, &A_local[j*X + X-2]);
                }
                for (i = 0; i <= X-1; ++i) {
                    write_pipe_block(p2, &A_local[(Y-2)*X + i]);
                }
            }
            else if (gid_x < M/(2*X)-1) {
                // compute
    ...
    if (gid_y == 0) {
        if (gid_x == 0) {
            for (j = 0; j <= Y-1; ++j) {
                async_work_group_copy(&A[(gid_y*Y+j)*M + gid_x*X], &A_local[X*j], X, 0);
            }
        }
        else if (gid_x < M/(2*X)-1) {
            for (j = 0; j <= Y-1; ++j) {
        ...
        if (gid_x == 0) {
            for (j = 0; j <= Y-1; ++j) {
                async_work_group_copy(&A[(gid_y*Y+j)*M + gid_x*X], &A_local[X*(j+T)], X, 0);
            }
        }
        else if (gid_x < M/(2*X)-1) {
            for (j = 0; j <= Y-1; ++j) {
